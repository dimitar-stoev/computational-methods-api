...
export function lagrangeInterpolation(
    ...
    const value = xValue !== undefined
        ? interpolatedValue.toFixed(decimalPlaces)
        : undefined;
    ...
    if (trueFunction && xValue !== undefined) {
        const trueValue = new Decimal(trueFunctionFn(xValue)).toFixed(decimalPlaces);
        console.log(`Стойности на сравняващата фунцкия: ${trueValue}`);

        const error = new Decimal(trueValue).sub(new Decimal(value));
        console.log("Грешка: ", error.toFixed(4));
